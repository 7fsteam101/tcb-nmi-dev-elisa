-- =====================================================================
-- 0038: fix finance.recompute_commissions enum casts (payout status +
-- commission_line type were inserted as text from the temp tables, which
-- 42804-errors at runtime). Function body otherwise identical to 0037.
-- =====================================================================
...
begin
  ...
  select distinct l.rep_id, l.period_month, (l.period_month + interval '1 month - 1 day')::date, 'calculated'::public.commission_payout_status, 0, p_is_demo
  from _line l
  ...
  select l.rep_id, null, l.deal_id, cp.id, l.line_type::public.commission_line_type, coalesce(l.rate_applied, 0), l.amount_minor, p_is_demo
  from _line l
