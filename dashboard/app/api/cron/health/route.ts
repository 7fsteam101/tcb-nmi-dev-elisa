...
export async function GET(req: NextRequest) {
  ...
  await attempt("parallel_8", () => Promise.all([
    sql`select count(*) from core.contact`, sql`select count(*) from sales.opportunity`,
    sql`select count(*) from sales.call`, sql`select count(*) from sales.appointment`,
    sql`select count(*) from sales.deal`, sql`select count(*) from core.dq_reason`,
    sql`select count(*) from core.lost_reason`, sql`select count(*) from marketing.pricing_plan`,
  ]));
