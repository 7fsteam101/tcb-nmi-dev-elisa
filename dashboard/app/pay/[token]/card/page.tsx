  ...
  return shell(
    <>
      ...
      <Checkout token={token} firstAmountLabel="" defaultName={link.customer_name || ""} defaultEmail={link.customer_email || ""}
        tokenizationKey={tokenizationKey} testMode={testMode} action="/api/update-card" payLabel="Update card" requireContact={false} collectZip={false} showWallets={false}
        successTitle="Card updated"
        successSubtitle="Your card on file has been updated. Your remaining scheduled payments will charge to it." />
    </>
  );
}
