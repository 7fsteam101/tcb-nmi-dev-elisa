...
// Shared contact-detail body — rendered by both the full page (/contacts/[id])
// and the half-screen drawer (intercepting route). Returns null-safe content.
...
export async function ContactBody({ id }: { id: string }) {
  const contactRows = await sql`
    ...
    where ct.id = ${id}`;
  if (contactRows.length === 0) return <None>Contact not found</None>;
  ...
  const [identifiers, opportunities, calls, appointments, deals, plans, receivables, payments, optIns, reports, objections] =
    await Promise.all([
      ...
      sql`select a.id, a.call_id, a.seq, a.scheduled_for, a.status, a.moved_by, r.name as reason from sales.appointment a left join core.cancellation_reason r on r.id = a.reason_id where a.call_id in (select c.id from sales.call c where c.opportunity_id in (select o.id from sales.opportunity o where o.contact_id = ${id})) order by a.seq`,
      sql`select d.id, d.plan_type_snapshot, d.total_contract_value_minor, d.status, d.deal_close_date from sales.deal d where d.contact_id = ${id} order by d.deal_close_date desc`,
      ...
      sql`select rs.id, rs.type, rs.submitted_at, rs.status from sales.report_submission rs where rs.strategy_call_id in (select c.id from sales.call c where c.opportunity_id in (select o.id from sales.opportunity o where o.contact_id = ${id})) or rs.follow_up_call_id in (select c.id from sales.call c where c.opportunity_id in (select o.id from sales.opportunity o where o.contact_id = ${id})) order by rs.submitted_at desc`,
      sql`select ob.id, ob.created_at, ob.led_to_loss, ob.source, ot.name as objection from sales.objection ob left join core.objection_type ot on ot.id = ob.objection_type_id where ob.strategy_call_id in (select c.id from sales.call c where c.opportunity_id in (select o.id from sales.opportunity o where o.contact_id = ${id})) order by ob.created_at desc`,
    ]);
  ...
  const primaryVals = new Set([contact.primary_email, contact.primary_phone].filter(Boolean).map((v) => String(v).toLowerCase()));
  ...
  return (
    <div>
            ...
            {oppCalls.map((c: any) => {
              ...
              return (
                <div key={c.id} className="mt-3 border-t pt-3" style={{ borderColor: "var(--line)" }}>
                  ...
                  {slots.length === 0 ? <p className="mt-2 text-sm" style={{ color: "var(--muted)" }}>No scheduled slots</p> : (
                    <div className="mt-2 overflow-x-auto">
                      <table>
                        <thead><tr><th>Slot</th><th>Scheduled for</th><th>Status</th><th>Reason</th><th>Moved by</th></tr></thead>
                        <tbody>
                          {slots.map((a: any) => (
                            <tr key={a.id}><td>#{a.seq}</td><td>{dateTime(a.scheduled_for, tz)}</td><td><Badge tone={tone(a.status)}>{label(a.status)}</Badge></td><td style={{ color: "var(--muted)" }}>{a.reason ?? "—"}</td><td className="capitalize" style={{ color: "var(--muted)" }}>{label(a.moved_by)}</td></tr>
                          ))}
                        </tbody>
                      </table>
                    </div>
        ...
        return (
          <Card key={d.id} className="mb-3">
              ...
              return (
                <div key={p.id} className="mt-3 border-t pt-3" style={{ borderColor: "var(--line)" }}>
                  ...
                  {recv.length > 0 && (
                    <div className="mt-2 overflow-x-auto">
                      <table>
                        <thead><tr><th>Installment</th><th>Due</th><th>Amount</th><th>Status</th></tr></thead>
                        <tbody>{recv.map((r: any) => <tr key={r.id}><td>#{r.installment_no}</td><td>{shortDate(r.due_date, tz)}</td><td>{money(r.amount_minor)}</td><td><Badge tone={tone(r.status)}>{label(r.status)}</Badge></td></tr>)}</tbody>
                      </table>
                    </div>
                  )}
                </div>
            ...
            <div className="mt-3 border-t pt-3" style={{ borderColor: "var(--line)" }}>
              {dealPayments.length === 0 ? <p className="text-sm" style={{ color: "var(--muted)" }}>No payments</p> : (
                <div className="overflow-x-auto">
                  <table>
                    <thead><tr><th>Paid</th><th>Type</th><th>Amount</th><th>Processor</th></tr></thead>
                    <tbody>{dealPayments.map((p: any) => <tr key={p.id}><td>{dateTime(p.occurred_at, tz)}</td><td className="capitalize" style={{ color: "var(--muted)" }}>{label(p.type)}</td><td>{money(p.amount_minor)}</td><td className="capitalize" style={{ color: "var(--muted)" }}>{label(p.processor)}</td></tr>)}</tbody>
                  </table>
                </div>
              )}
            </div>
      ...
      {optIns.length === 0 ? <None /> : (
        <Card><table><thead><tr><th>Submitted</th><th>Source</th><th>Campaign</th></tr></thead><tbody>{optIns.map((o: any) => <tr key={o.id}><td>{dateTime(o.submitted_at, tz)}</td><td>{o.source_channel ?? "—"}</td><td>{o.source_campaign ?? "—"}</td></tr>)}</tbody></table></Card>
      )}

      <SectionTitle>Report submissions</SectionTitle>
      {reports.length === 0 ? <None /> : (
        <Card><table><thead><tr><th>Submitted</th><th>Type</th><th>Status</th></tr></thead><tbody>{reports.map((r: any) => <tr key={r.id}><td>{dateTime(r.submitted_at, tz)}</td><td className="capitalize" style={{ color: "var(--muted)" }}>{label(r.type)}</td><td><Badge tone={tone(r.status)}>{label(r.status)}</Badge></td></tr>)}</tbody></table></Card>
      )}

      <SectionTitle>Objections</SectionTitle>
      {objections.length === 0 ? <None /> : (
        <Card><table><thead><tr><th>Logged</th><th>Objection</th><th>Led to loss</th><th>Source</th></tr></thead><tbody>{objections.map((ob: any) => <tr key={ob.id}><td>{dateTime(ob.created_at, tz)}</td><td>{ob.objection ?? "—"}</td><td>{ob.led_to_loss ? <Badge tone="bad">yes</Badge> : "no"}</td><td className="capitalize" style={{ color: "var(--muted)" }}>{label(ob.source)}</td></tr>)}</tbody></table></Card>
      )}
    </div>
  );
}
