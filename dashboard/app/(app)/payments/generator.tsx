...
import { createPaymentLinkAction, searchContactsAction } from "./actions";
...
export function LinkGenerator({
  products, canPickStripe,
}: {
  products: ProductOption[]; canPickStripe: boolean;
}) {
  ...
  // Debounced server-side contact search (replaces the old preloaded-500 + client
  // filter that left ~8k contacts unreachable). Fires ~250ms after the last
  // keystroke; the `active` flag drops stale responses so fast typing can't render
  // an earlier query's results over a newer one. Under 2 chars: no query.
  useEffect(() => {
    const q = query.trim();
    ...
    const t = setTimeout(async () => {
      try {
        const r = await searchContactsAction(q);
      ...
      <form action={action} className="space-y-3">
        ...
        <div ref={ref} className="relative">
          ...
          {selected ? (
            ...
            <>
              <input type="text" value={query} placeholder="Search contacts by name, email, or phone"
              ...
              {open && (
                <div className="absolute z-30 mt-1 max-h-64 w-full overflow-auto rounded-xl border p-1 shadow-xl" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
                  {query.trim().length < 2 ? (
                    <div className="px-3 py-2 text-[13px]" style={{ color: "var(--muted)" }}>Type at least 2 characters to search</div>
                  ) : searching ? (
                    <div className="px-3 py-2 text-[13px]" style={{ color: "var(--muted)" }}>Searching…</div>
                  ) : results.length === 0 ? (
                    <div className="px-3 py-2 text-[13px]" style={{ color: "var(--muted)" }}>No matching contacts</div>
                  ) : (
                    results.map((c) => (
                      <button key={c.id} type="button" onClick={() => { setSelected(c); setQuery(""); setResults([]); setOpen(false); }}
